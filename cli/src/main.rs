...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::ActorDump { pretty, out } => {
            let actor = sample_fighter();
            let s = if pretty { serde_json::to_string_pretty(&actor)? } else { serde_json::to_string(&actor)? };
            if let Some(path) = out { fs::write(path, s.as_bytes())?; } else { println!("{}", s); }
        }
        ...
        Cmd::AttackDemo { ac, dice, seed, adv, file } => {
            ...
            println!("attack: roll={} bonus={:+} total={} vs ac={} => {}{}",
                atk.roll, atk.bonus, atk.total, atk.ac,
                if atk.hit { "HIT" } else { "MISS" },
                if atk.nat20 { " (CRIT)" } else if atk.nat1 { " (NAT1)" } else { "" }
            );
            println!("damage: {} + {:+}{} => {}",
                dice, damage_mod,
